 ...
 * 调用  ：被外部调用
 */
bool ESP8266_JoinAP ( char * pSSID, char * pPassWord )  //为什么形参是指针变量? 
	//引入参数是字符串，字符串相当于是数组形式存储，参数定为指针变量赋给的字符串首字符的地址，所以后面要引用的就是地址后面所带的字符串内容 
{
	char cCmd [120];
 ...
 * 调用  ：被外部调用
 */
bool ESP8266_Link_Server ( ENUM_NetPro_TypeDef enumE, char * ip, char * ComNum, ENUM_ID_NO_TypeDef id)//同ESP8266_JoinAP函数，为什么这里也是用的指针变量
{
